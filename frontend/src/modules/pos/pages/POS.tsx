            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filtered.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 hover:border-vixo-300 dark:hover:border-vixo-700 transition-all">
                  <div>
                    <div className="font-medium text-slate-900 dark:text-slate-50">{p.nombre}</div>
                    <div className="text-sm text-slate-600 dark:text-slate-400">${p.precio.toFixed(2)}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
              ...
              {items.map((it, idx) => (
                <li key={it.id} className="flex items-center justify-between">
                  <div className="max-w-[55%]">
                    <div className="font-medium text-slate-900 dark:text-slate-50">{it.nombre}</div>
                    <div className="text-sm text-slate-600 dark:text-slate-400">${it.precio.toFixed(2)}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
