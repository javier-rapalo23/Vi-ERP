            ...
            {data && data.length > 0 ? (
              data.map((p) => (
                <tr key={p.id} className="border-t border-slate-200 dark:border-slate-800 hover:bg-vixo-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-4 py-3 text-slate-900 dark:text-slate-50 font-medium">{p.codigo}</td>
                  <td className="px-4 py-3 text-slate-900 dark:text-slate-50">{p.nombre}</td>
                  <td className="px-4 py-3 text-right text-slate-700 dark:text-slate-300">${p.precio.toFixed(2)}</td>
                  <td className="px-4 py-3 text-right text-slate-700 dark:text-slate-300">${p.costo.toFixed(2)}</td>
                  <td className="px-4 py-3 text-right text-slate-700 dark:text-slate-300">{p.stock}</td>
                </tr>
              ))
            ) : (
              <tr>
